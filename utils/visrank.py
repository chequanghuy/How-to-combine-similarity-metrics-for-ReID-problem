...
def visualize_ranked_results(
    ...
    for q_idx in range(500):
        qimg, qpid, qcamid, qimg_path = dataset[q_idx]
        qimg_path_name = (
        ...
        for g_idx in indices[q_idx, :]:
            gimg, gpid, gcamid, gimg_path = dataset[g_idx+num_q]
        ...
        if good==0:
            ...
            ap = score/good
        AP_percentage = ap * 100
        ap_text = f'AP: {AP_percentage:.2f}%'
        ap_img = 255 * np.ones((height, 3 * width, 3), dtype=np.uint8)  # 4 times longer width

        # Calculate the position to center the text
        text_size = cv2.getTextSize(ap_text, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)[0]
        text_x = (3 * width - text_size[0]) // 2
        text_y = (height + text_size[1]) // 2

        cv2.putText(ap_img, ap_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (0, 0, 0), 2, cv2.LINE_AA)

        # Append the 4 times longer width AP information as a new grid box at the end of grid_img
        grid_img[:, -ap_img.shape[1]:, :] = ap_img
        if data_type == "image":
